...
class Elasticity(FEM):
    def get_tensor_map_box(self):
        def stress(u_grad, theta):
            ...
            sigma = lmbda*np.trace(epsilon)*np.eye(self.dim) +\
                  2*mu*epsilon
            return sigma
        return stress
    
    def compute_von_mises_stress(self, sol):
        # (num_cells, 1, num_nodes, vec, 1) * (num_cells, num_quads,
        ...
        vm_stress = jax.vmap(jax.vmap(vm_stress_fn))(u_grads,\
             *self.internal_vars['laplace']) # (num_cells, num_quads)
        volume_avg_vm_stress = np.sum(vm_stress * self.JxW, axis=1) /\
    ...
    def compute_4_points_polygon_area(self, A, B, C, D):
        '''
        D---C
        |   |
        A---B
        ...
        area = 0.0
        for i in range(4):
            area += xs[i] * ys[(i + 1)%4] - xs[(i + 1)%4] * ys[i]
        area *= .5

        return abs(area)

    def compute_4_points_polygon_centroid(self, A, B, C, D):
        '''
        D---C
        |   |
        A---B
        ...
        abs_area = self.compute_4_points_polygon_area(A, B, C, D)

        c_x = 0.0
        c_y = 0.0

        for i in range(4):
            c_x += (xs[i] + xs[(i + 1)%4]) * (xs[i] * ys[(i + 1)%4] - xs[(i + 1)%4] * ys[i])
        c_x /= 6 * abs_area

        for i in range(4):
            c_y += (ys[i] + ys[(i + 1)%4]) * (xs[i] * ys[(i + 1)%4] - xs[(i + 1)%4] * ys[i])
        c_y /= 6 * abs_area
