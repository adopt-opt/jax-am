...
class TopOptEnv(gym.Env):
    ...
    def _get_obs(self):
        self._strains, self._bounds, self._forces = self.state_tensor[0,:,:], self.state_tensor[1,:,:], self.state_tensor[2,:,:]
    ...
    def reset(self, seed=123, options=123):
        ...
        self.state_tensor = self.jax_model.create_state_space_tensor(self.rho_matrix, self.initial_von_mises, self.bounded_cells, self.loaded_cells)
        
        self.prev_state_tensor = self.state_tensor
    ...
    def step(self, action):
        ...
        print(f'TENSOR shape: {self.state_tensor.shape}')
        reward = 0
        
        if self.jax_model.check_illegal(self.rho_matrix, cell_to_be_removed, self.state_tensor, self.nb_removed_cells):
            reward = -1
            terminated = True
            ...
            self.state_tensor = self.jax_model.create_state_space_tensor(rho_matrix, von_mises, self.bounded_cells, self.loaded_cells)
            indices = onp.argwhere(self.jax_model.cell_inds_matrix==cell_to_be_removed)
            index_x, index_y = indices[0][0], indices[0][1]
            self._remove_cell_color(index_x, index_y)
        ...
        return self.prev_state_tensor, action, reward, self.state_tensor, terminated
        #return state_tensor, reward, terminated, False , {}
