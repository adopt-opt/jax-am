...
class ProblemSetup(Elasticity):
    ...
    def create_state_space_tensor(self, rho_vector: np.ndarray, von_mises: np.ndarray, bounded_cell_inds:np.ndarray, loaded_cell_inds: np.ndarray) -> np.ndarray:
        ...
        return np.stack((inverse_von_mises_matrix, bounded_cells_state_matrix, loaded_cells_state_matrix))
    
    def check_illegal(self, rho_matrix: np.ndarray, new_point: int, state_matrix: np.ndarray, nb_step: int) -> bool:
        """
    ...
    def positive_reward(self, initial_von_mises: float, current_von_mises: float, num_of_voided_cells: int, num_of_total_cells: int) -> float:
        ...
        """
        return np.sqrt(np.sum(current_von_mises) / np.sum(initial_von_mises)) + np.sqrt(num_of_voided_cells / num_of_total_cells)
...
if __name__ == "__main__":
    ...
    num_of_cells = Nx * Ny
    vf = 1
    init_rho_vector = vf*np.ones((num_of_cells, 1)) 
    ...
    while episode_counter <= num_episodes:
        ...
        while step_counter <= num_steps:
            # update step part
            step_counter += 1
            state_tensor = trial.create_state_space_tensor(rho_matrix, von_mises, bounded_cells, loaded_cells)
            cell_to_be_removed = onp.random.choice(onp.arange(num_of_cells), 1)
            illegality_check = trial.check_illegal(rho_matrix, cell_to_be_removed, state_tensor)
            if not illegality_check:
                reward -= 1
    ...
    print(f"Max_reward = {max(reward_history)}, Max_reached_step = {max(step_history)} out of total {num_steps} steps!")
    print()
