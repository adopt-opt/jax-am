...
class ProblemSetup(Elasticity):
    def __init__(self, Nx, Ny, Lx, Ly, num_bounded_cell=2,
     num_loaded_cell=1, filled_density=1., void_density=0., dim=2,
     vec=2):
    ...
    def select_bounded_and_loaded_points(self, bounded_cell_inds:
         np.ndarray, loaded_cell_inds: np.ndarray) -> list:
        """
    ...
    def create_state_space_tensor(self, rho_vector: np.ndarray,
    von_mises: np.ndarray, bounded_cell_inds:np.ndarray,
    loaded_cell_inds: np.ndarray) -> np.ndarray:
        """
        Creates required DQN input 4 x N x N  state tensor 
        Args:
        ...
        Returns:
            state_tensor_DQN                : NxNx4 tensor which will be used in DQN training
            state_tensor_check              : 4xNxN tensor used for illegality check and visualization
        """
        #inverse_von_mises_array = (1 / von_mises) / np.max(1 / von_mises)

        #print(f'rho vector shjape : {rho_vector.shape}')
        #print(f'von_mises shape: {von_mises.shape}')
        #inverse_von_mises_array = von_mises
        inverse_von_mises_array = np.zeros_like(von_mises)
        max_VM = np.max(von_mises)
        print(f'max_VM here :{max_VM} at {np.argmax(von_mises)}')
        for i in range(len(von_mises)):
            if rho_vector[i] > self.void_density:
                inverse_von_mises_array = inverse_von_mises_array.at[i].set(max_VM/von_mises[i])

        # high_val_indx_list = []
        # for i in range(len(inverse_von_mises_array)):
        #     if inverse_von_mises_array[i] > 1e3:
        #         inverse_von_mises_array = inverse_von_mises_array.at()

        inverse_von_mises_array = inverse_von_mises_array.at[inverse_von_mises_array>1e3].set(0.)
        inverse_von_mises_array = inverse_von_mises_array/np.max(inverse_von_mises_array)
        
        
        
        # max_inverse = -10000
        # max_inverse_indx = 0
        # print(f'Von mises Stresses : {von_mises}')
        # for i in range( len(von_mises)):
        #         if von_mises[i] >1e-6:
        #             if 1/von_mises[i] > max_inverse:
        #                 max_inverse = 1/von_mises[i]
        #                 max_inverse_indx = i
        
        # print(f'MAX INVERSE stress: {max_inverse} at {max_inverse_indx}')
        # for i in range( len(von_mises)):
        #         if von_mises[i] > 1e-6:
        #             inverse_von_mises_array = inverse_von_mises_array.at[i].set((1/von_mises[i])/max_inverse) 
        
        # for i in range(len(von_mises)):
        #     if np.isnan(inverse_von_mises_array[i]):
        #         print(f'Nan value encountered at {i}')
        #         exit(1)
        ...
        voided_cells_state_matrix = self._state_matrix_from_array(rho_vector > self.void_density, self.Nx, self.Ny)
        state_tensor_DQN = np.stack((inverse_von_mises_matrix, bounded_cells_state_matrix, loaded_cells_state_matrix, voided_cells_state_matrix), axis=2)
        state_tensor_check = np.stack((inverse_von_mises_matrix, bounded_cells_state_matrix, loaded_cells_state_matrix, voided_cells_state_matrix), axis=0)
        return state_tensor_DQN, state_tensor_check
    
    def check_illegal(self, rho_matrix: np.ndarray, new_point: int, 
    state_tensor: np.ndarray, nb_step: int, nb_max_step: int) -> bool:
        """
        ...
        """
        _, bounds, forces, _ = state_tensor
    ...
    def update_density(self, rho_vector: np.ndarray, 
    ...
    def positive_reward(self, strain_energy_initial: float,
    ...
    num_of_total_cells: int) -> float:
        ...
        return ( strain_energy_initial / strain_energy_current) ** 2 + (num_of_voided_cells / num_of_total_cells) ** 2
...
if __name__ == "__main__":
    ...
    while episode_counter <= num_episodes:
        ...
        print(solution)
        exit()
        disp = solution.reshape((Nx+1, Ny+1, -1))
        disp = solution[::-1, :,:]
        # solution = np.arange(36)
        # solution = solution.reshape((6,6), order="F")
        # print(f'Reshaped Solution: { solution} ')
        # solution = solution[::-1, :]
        # print(solution)
        # exit()
        #print(f'Changed Solution shape: {solution.shape}')
